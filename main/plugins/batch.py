...
async def _batch(event):
    ...
    if f'{event.sender_id}' in batch:
        return await event.reply("BSDK RUK JA BKL 1 TO START KRRKHA NA CHUTIYE!")
    async with gagan.conversation(event.chat_id) as conv: 
        if not s:
            ...
            cd = await conv.send_message("**Batch process ongoing...**\n\nProcess completed: ", 
                                    buttons=[[Button.url("Join Channel", url="http://t.me/PRAGYANCODER")]])
    ...
    for i in range(len(ids)):
        ...
        try: 
            ...
            if countdown.text != count_down:
                await countdown.edit(count_down, buttons=[[Button.url("Join Channel", url="https://t.me/PragyanCoder")]])
        n = i + 1
        if n == len(ids):
            return -2
...
async def start_command(event):
    ...
    buttons = [
        [Button.inline("Cancel", data="cancel"),
         Button.inline("Cancel", data="cancel")],
        [Button.url("Join Channel", url="https://telegram.dog/PragyanCoder")]
    ]
